  ...
  return (
    <div className="flex">
      ...
      <div className="flex h-screen w-screen bg-gray-100 p-2 pt-20">
        <div className="w-1/3 p-4 bg-[#6666ff] shadow-lg rounded-lg h-full overflow-auto">
          <h2 className="text-2xl font-bold text-black mb-3 mt-4">Notlar</h2>
          {selectedDate ? (
            <>
              <h3 className="text-lg font-bold mb-2">Not Ekle ({selectedDate})</h3>
              <textarea
              ...
              {Array.isArray(notes[selectedDate]) && notes[selectedDate].length > 0 && (
                <div className="mt-3 w-full bg-gray-200 p-3 rounded-lg shadow">
                  <h3 className="text-lg font-bold mb-2 text-black">Kaydedilen Notlar</h3>
                  <ul>
                    {notes[selectedDate].map((note, index) => (
                      <li key={index} className="flex justify-between items-center border-b py-1">
                        <span className="text-black text-sm">{note.todo}</span>
                        <button
            ...
            <p className="text-black text-sm">Bir gün seçerek not ekleyebilirsin. 📌</p>
          )}
        </div>
        <div className="flex flex-col items-center justify-start w-2/3 p-2 pt-12">
          <div className="flex items-center justify-between w-full max-w-3xl mb-4">
            ...
            <h2 className="text-xl font-bold text-black">{currentMonth.format("MMMM YYYY")}</h2>
            <button
              onClick={() => changeMonth(1)}
              className="px-3 py-1 bg-gray-800 text-white rounded-lg hover:bg-gray-700 text-sm"
            >
          ...
          <div className="grid grid-cols-7 gap-1 w-full max-w-3xl mb-1">
            {weekDays.map((day) => (
              <div key={day} className="text-center text-sm font-bold text-black">
                {day}
              </div>
            ))}
          </div>
